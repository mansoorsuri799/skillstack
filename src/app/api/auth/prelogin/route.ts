...
export async function POST(request: Request) {
  try {
    ...
    if (/ECONNREFUSED|ENOTFOUND|MongoServerSelectionError|authentication failed|bad auth/i.test(message)) {
      return NextResponse.json(
        {
          error: "DATABASE",
          message:
            "Cannot reach MongoDB. Check MONGODB_URI in .env.local and Atlas network access.",
        },
        { status: 503 },
      );
    }
