...
function MessageBubble({ msg }: { msg: ChatMessage }) {
  ...
  return (
    <div className={`flex w-full gap-3 md:gap-4 ${isUser ? "justify-end" : "justify-start"}`}>
      {!isUser ? (
        <div className="flex h-8 w-8 shrink-0 select-none items-center justify-center rounded-full border border-[#388bfd]/40 bg-[#388bfd]/10 text-[#58a6ff]">
          <Bot className="h-4 w-4" />
        </div>
      ) : null}

      <div
        className={`relative max-w-[85%] rounded-2xl p-4 text-sm leading-relaxed transition-all md:max-w-[75%] ${
          isUser
            ? "border border-white/10 bg-[#161b22] text-snow font-medium"
            : "border border-line/60 bg-[#0d1117] text-ink"
        }`}
      >
            ...
            <div className="grid gap-1.5 sm:grid-cols-2">
              {msg.sources.map((s, idx) => (
                <a
                  ...
                  className="flex items-center justify-between gap-2 rounded-lg border border-line/70 bg-bg px-2.5 py-1.5 text-xs text-snow/90 transition hover:border-accent/40 hover:text-accent"
                >
      ...
      {isUser ? (
        <div className="flex h-8 w-8 shrink-0 select-none items-center justify-center rounded-full border border-line bg-[#161b22] text-ink-muted">
...
export default function ChatPage() {
  ...
  // Load chat session if ID changes in URL
  ...
  async function handleSend(customPrompt?: string) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "Failed to get reply from SAM");
      ...
      setError(err instanceof Error ? err.message : "SAM query failed");
    } finally {
      setLoading(false);
  ...
  return (
    <DashboardShell title="SAM SEO Agent">
      <PageStack className="max-w-5xl mx-auto flex flex-col min-h-[calc(100vh-130px)]">
        ...
        <div className="flex items-center justify-between border-b border-line/60 pb-3 text-xs">
          ...
          <div className="relative">
            ...
            {memoryOpen ? (
              <div className="absolute right-0 top-full mt-2 w-72 rounded-xl border border-line bg-[#0d1117] p-3 text-xs shadow-2xl z-50 space-y-2">
                <div className="flex items-center justify-between border-b border-white/[0.06] pb-1.5 font-semibold text-snow">
                  <span>Project Context</span>
                  <span className="text-[10px] text-accent">Active Sync</span>
                </div>
                ...
                <p className="text-[11px] text-ink-muted/70 pt-1 border-t border-white/[0.04]">
                  SAM uses this project metadata to tailor all keyword, SERP, and traffic answers specifically for {project?.domain}.
                </p>
              </div>
            ) : null}
          </div>
        ...
        <div className="flex-1 space-y-4 py-4 overflow-y-auto">
          {messages.length === 0 ? (
            <div className="max-w-2xl mx-auto py-12 text-center space-y-6">
              ...
              {/* Quick Starter Pill Buttons (Exact matching screenshot) */}
              <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
                {QUICK_STARTERS.map((starter, idx) => (
                  <button
                    ...
                    className="rounded-full border border-[#30363d] bg-[#161b22]/70 px-4 py-2 text-xs font-medium text-snow/90 transition hover:border-[#58a6ff] hover:bg-[#161b22] hover:text-[#58a6ff]"
                  >
                    {starter}
                  </button>
                ))}
          ...
          {loading ? (
            <div className="flex items-center gap-3 rounded-2xl border border-line bg-[#161b22] p-4 text-xs text-ink-muted animate-pulse max-w-md">
              <Bot className="h-4 w-4 text-accent animate-spin" />
              <span>SAM is analyzing project metrics and SERPs...</span>
            </div>
          ) : null}
        ...
        {/* Bottom Input Pill matching exact user screenshot */}
        <div className="sticky bottom-0 pt-2 pb-4 bg-gradient-to-t from-bg via-bg to-transparent">
          <div className="relative flex items-center rounded-2xl border border-[#30363d] bg-[#0d1117] px-4 py-2 shadow-2xl transition focus-within:border-[#388bfd] focus-within:ring-1 focus-within:ring-[#388bfd]/30">
            <textarea
              ref={textareaRef}
              value={input}
            ...
            <button
              ...
              className={`absolute right-3 flex h-8 w-8 items-center justify-center rounded-full transition ${
                input.trim() && !loading
                  ? "bg-[#2f81f7] text-white hover:bg-[#1f6feb] shadow-md cursor-pointer"
                  : "bg-[#21262d] text-[#8b949e] cursor-not-allowed opacity-50"
              }`}
              title="Send to SAM (Enter)"
            >
